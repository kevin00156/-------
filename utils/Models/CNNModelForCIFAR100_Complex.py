...
# 定義 CNN 模型結構
class CNNModelForCIFAR100_Complex(nn.Module):
    def __init__(self,
                 input_size: int = 64, 
                 num_classes: int = 100,
                 hidden_layers: Sequence[int] = [512, 256, 128, 64],
                 activation_function: nn.Module = nn.LeakyReLU(),
                 dropout_rate: float = 0.3):
        super(CNNModelForCIFAR100_Complex, self).__init__()
...
if __name__ == "__main__":
    model = CNNModelForCIFAR100_Complex()
    print(model)
